...
from ml.churn import (
    ...
    get_local_explanation, detect_drift, detect_anomalies
)
from ml.segment import segment_customers, get_segment_summary
from ml.clv import predict_clv, is_clv_trained
...
def predict_single(
    ...
    try:
        ...
        prediction_record = models.PredictionHistory(
            customer_id=input_dict.get("customer_id", "WHAT_IF_SIM"),
            model_version="2.0-masterpiece",
            inputs=input_dict,
        ...
        if "customer_id" not in input_dict or input_dict["customer_id"] == "WHAT_IF_SIM":
            audit = models.AuditLog(
                user_email=user["email"],
...
def clv_predictions(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    if not is_clv_trained():
        # Trigger async training if not available
